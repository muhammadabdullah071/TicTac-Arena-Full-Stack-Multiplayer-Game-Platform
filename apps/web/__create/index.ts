...
let adapterReady = new Promise<ReturnType<typeof NeonAdapter> | ReturnType<typeof InMemoryAdapter>>(async (resolve) => {
  if (!process.env.DATABASE_URL) {
    console.warn('[auth] No DATABASE_URL set, using in-memory adapter');
    resolve(InMemoryAdapter());
    return;
  }
  ...
  try {
    ...
    client.release();
    console.log('[auth] PostgreSQL connection OK');
    resolve(NeonAdapter(pool));
  } catch (err) {
    console.warn('[auth] PostgreSQL unavailable, using in-memory adapter:', (err as Error)?.message);
    resolve(InMemoryAdapter());
  }
});
